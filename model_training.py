...
# Example mapping for company and models (replace this with your actual data)
company_to_models = {
   'Ambassador': [
...
# Extract 'company' and 'model' from 'name'
df['company'] = df['name'].apply(lambda x: next((company for company in company_to_models if company in x), 'Other'))
df['model'] = df.apply(
    lambda row: next((model for model in company_to_models.get(row['company'], []) if model in row['name']), 'Unknown'),
    axis=1
)

# Select features and target variable
X = df[['year', 'km_driven', 'fuel', 'transmission', 'seller_type', 'owner', 'company', 'model']]
y = df['selling_price']

# OneHotEncode categorical variables
categorical_features = ['fuel', 'transmission', 'seller_type', 'owner', 'company', 'model']
...
print("Model training complete! Pipeline saved as 'car_price_model.pkl'.")
